...
static func _decompose_syllable(syllable: String):
...
static func compose_removal(s: String):
	if s == "":
		return # Back to default handling
	var lastChar = s[-1]
...
static func ends_with_vowel(s: String, include_rieul: bool = false):
	if s == "":
		return false
