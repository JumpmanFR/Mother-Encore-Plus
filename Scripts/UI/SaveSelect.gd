...
func _physics_process(delta):
	if _active:
		...
		if direction < 0 and _index > 1:
			...
			if !_cursor_top:
				_cursor_top = true
				create_tween().tween_property(_menus_cursor, "rect_position:y", 0, 0.2) \
						.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
			else:
				var new_saves_offset = _saves_offset + _save_file_height
				create_tween().tween_property($CanvasLayer/Body/Saves, "rect_position:y", new_saves_offset, 0.2) \
						.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
			...
			tween.tween_property($CanvasLayer/Body/Saves, "rect_position:y", new_saves_offset, 0.2) 
			_saves_offset = new_saves_offset
			...
			if _cursor_top:
				_cursor_top = false
				create_tween().tween_property(_menus_cursor, "rect_position:y", _save_file_height, 0.2) \
						.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
			else:
				var new_saves_offset = _saves_offset - _save_file_height
				create_tween().tween_property($CanvasLayer/Body/Saves, "rect_position:y", new_saves_offset, 0.2) \
