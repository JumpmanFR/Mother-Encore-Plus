...
onready var spark = preload("res://Nodes/Reusables/Overlap/BeamSpark.tscn")
onready var sfx_shoot = load("res://Audio/Sound effects/Lloyd Beam.mp3")
onready var sfx_hit = load("res://Audio/Sound effects/Laser Hit.wav")


func _ready():
	animationTree.active = true
	animationTree.set("parameters/shoot/blend_position", inputVector)
	audioManager.play_sfx(sfx_shoot, "Beam")
...
func _on_Area2D_area_entered(area):
	# check if the area's parent is a mirror
	var mirror = area.get_parent()
	if area.get_parent().get_groups().has("BeamReflector"):
		reflect(mirror)
	elif !area.get_groups().has("BeamArea"):
		disappear()
...
func disappear(silent: = false):
	if not silent:
		audioManager.play_sfx(sfx_hit, "BeamSpark")
